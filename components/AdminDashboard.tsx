    ...
    <div className="flex h-screen bg-white overflow-hidden text-slate-900 font-ubuntu">
      ...
      <aside className="w-80 bg-slate-950 m-4 rounded-[2.5rem] text-white flex flex-col p-8 shadow-2xl z-30 overflow-hidden relative shrink-0">
        <div className="mb-12 flex items-center gap-4 relative z-10">
          ...
          <div><span className="font-black text-xl tracking-tighter uppercase leading-none block">ATHLETIC</span><span className="text-[9px] font-bold text-blue-400 uppercase tracking-widest">Staff Admin</span></div>
        </div>
        ...
        <button onClick={onLogout} className="mt-auto w-full flex items-center gap-4 px-6 py-4 text-rose-400 font-black text-[10px] uppercase tracking-widest hover:bg-rose-500/10 rounded-2xl transition-all"><LogOut size={20}/> Salir</button>
      </aside>

      {/* MAIN CONTENT */}
      <main className="flex-grow overflow-y-auto p-12 bg-slate-50 relative">
        <header className="flex justify-between items-end mb-16">
          <div><p className="text-blue-600 font-black text-[10px] uppercase tracking-[0.3em] mb-2">Administrador</p><h1 className="text-5xl font-black uppercase tracking-tighter text-slate-900 leading-none">{activeTab === 'students' ? 'Alumnos' : activeTab === 'content' ? 'Contenidos' : activeTab === 'settings' ? 'Ajustes' : activeTab}</h1></div>
          ...
          {activeTab === 'overview' && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="grid md:grid-cols-3 gap-8">
              <div className="bg-white p-12 rounded-[3rem] shadow-sm border border-slate-100 flex flex-col group hover:border-blue-200 transition-all">
                <Users size={32} className="text-blue-600 mb-6"/><p className={labelClasses}>Matrículas Activas</p><p className="text-7xl font-black tracking-tighter">{students.length}</p>
              </div>
              <div className="bg-white p-12 rounded-[3rem] shadow-sm border border-slate-100 flex flex-col group hover:border-emerald-200 transition-all">
                <DollarSign size={32} className="text-emerald-600 mb-6"/><p className={labelClasses}>Recaudación S/</p><p className="text-7xl font-black tracking-tighter">{students.reduce((acc, s) => acc + (s.total_paid || 0), 0)}</p>
              </div>
              <div className="bg-white p-12 rounded-[3rem] shadow-sm border border-slate-100 flex flex-col group hover:border-rose-200 transition-all">
                <AlertCircle size={32} className="text-rose-600 mb-6"/><p className={labelClasses}>Saldos Deudores S/</p><p className="text-7xl font-black tracking-tighter">{students.reduce((acc, s) => acc + (s.pending_balance || 0), 0)}</p>
              </div>
            </motion.div>
          )}
          ...
          {activeTab === 'students' && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
              <div className="flex flex-col md:flex-row items-center gap-6 bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100">
                ...
                <button onClick={() => setShowRegisterModal(true)} className="px-8 py-4 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase shadow-xl flex items-center gap-3 active:scale-95"><Plus size={20}/> Nuevo Alumno</button>
              </div>
              <div className="bg-white rounded-[3rem] shadow-sm overflow-hidden border border-slate-100">
                <table className="w-full text-left">
                  <thead className="bg-slate-50 text-slate-400 border-b border-slate-100"><tr className="text-[10px] font-black uppercase tracking-widest"><th className="p-8">Alumno</th><th className="p-8">Apoderado</th><th className="p-8 text-center">Estado Pago</th><th className="p-8 text-center">Acciones</th></tr></thead>
                    ...
                    {filteredStudents.map(s => (
                      <tr key={s.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="p-8 text-center"><div className="flex items-center justify-center gap-2">
                          ...
                          <button title="WhatsApp" onClick={() => window.open(`https://wa.me/51${s.parentPhone}?text=Hola%20${s.parentName},%20te%20escribimos%20de%20Athletic%20Academy.`, '_blank')} className="p-3 bg-emerald-100 text-emerald-700 rounded-xl hover:bg-emerald-700 hover:text-white transition-all"><MessageCircle size={18}/></button>
          ...
          {activeTab === 'content' && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-12 pb-32">
              <div className="bg-white p-12 rounded-[3rem] shadow-sm border border-slate-100 space-y-8">
                <div className="flex justify-between items-center"><h3 className="font-black text-slate-900 uppercase italic text-xl">Imágenes Hero (Inicio)</h3><button onClick={() => addItem('heroImages', '')} className="text-blue-600 text-[10px] font-black uppercase flex items-center gap-2 hover:scale-105 transition-all"><PlusCircle size={20}/> Añadir Imagen</button></div>
              ...
              <div className="bg-white p-12 rounded-[3rem] shadow-sm border border-slate-100 space-y-8">
                <div className="flex justify-between items-center"><h3 className="font-black text-slate-900 uppercase italic text-xl">Fotos Sección "Nosotros"</h3><button onClick={() => addItem('aboutImages', '')} className="text-blue-600 text-[10px] font-black uppercase flex items-center gap-2 hover:scale-105 transition-all"><PlusCircle size={20}/> Añadir Foto</button></div>
          ...
          {activeTab === 'settings' && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-12 pb-32">
              <div className="bg-white p-12 rounded-[3rem] shadow-sm border border-slate-100 grid md:grid-cols-2 gap-8">
                ...
                <div><label className={labelClasses}>WhatsApp Público</label><input value={localConfig.socialWhatsapp} onChange={e => setLocalConfig({...localConfig, socialWhatsapp: e.target.value})} className={inputClasses}/></div>
                ...
                <div className="md:col-span-2"><label className={labelClasses}>Sede / Dirección</label><input value={localConfig.contactAddress} onChange={e => setLocalConfig({...localConfig, contactAddress: e.target.value})} className={inputClasses}/></div>
              </div>

              <div className="bg-white p-12 rounded-[3rem] shadow-sm border border-slate-100 grid md:grid-cols-2 gap-8">
                <div className="md:col-span-2 flex items-center gap-4 text-emerald-600"><PlusCircle/><h3 className="font-black text-slate-900 uppercase italic tracking-tighter text-xl">Enlaces de Redes Sociales</h3></div>
                <div className="flex items-center gap-4"><Facebook className="text-blue-600 shrink-0"/><input value={localConfig.socialFacebook} onChange={e => setLocalConfig({...localConfig, socialFacebook: e.target.value})} className={inputClasses} placeholder="https://facebook.com/..."/></div>
                <div className="flex items-center gap-4"><Instagram className="text-pink-600 shrink-0"/><input value={localConfig.socialInstagram} onChange={e => setLocalConfig({...localConfig, socialInstagram: e.target.value})} className={inputClasses} placeholder="https://instagram.com/..."/></div>
                <div className="flex items-center gap-4"><Music2 className="text-slate-900 shrink-0"/><input value={localConfig.socialTiktok} onChange={e => setLocalConfig({...localConfig, socialTiktok: e.target.value})} className={inputClasses} placeholder="https://tiktok.com/..."/></div>
              </div>

              <div className="bg-white p-12 rounded-[3rem] shadow-sm border border-slate-100 grid md:grid-cols-2 gap-8">
                <div className="md:col-span-2 flex items-center gap-4 text-emerald-600"><CreditCard/><h3 className="font-black text-slate-900 uppercase italic tracking-tighter text-xl">Datos de Pago (Visibles en Web)</h3></div>
                <div><label className={labelClasses}>Yape (N° Celular)</label><input value={localConfig.yapeNumber} onChange={e => setLocalConfig({...localConfig, yapeNumber: e.target.value})} className={inputClasses}/></div>
                <div><label className={labelClasses}>Yape (Nombre de Titular)</label><input value={localConfig.yapeName} onChange={e => setLocalConfig({...localConfig, yapeName: e.target.value})} className={inputClasses}/></div>
                <div className="md:col-span-2 h-[1px] bg-slate-100"></div>
                <div><label className={labelClasses}>BCP (Número de Cuenta)</label><input value={localConfig.bcpAccount} onChange={e => setLocalConfig({...localConfig, bcpAccount: e.target.value})} className={inputClasses}/></div>
                <div><label className={labelClasses}>BCP (Nombre de Titular)</label><input value={localConfig.bcpName} onChange={e => setLocalConfig({...localConfig, bcpName: e.target.value})} className={inputClasses}/></div>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
          ...
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative w-full max-w-2xl bg-white rounded-[3rem] p-12 shadow-2xl">
              ...
              <div className="flex gap-8 items-start mb-10">
                <div className="w-24 h-24 bg-slate-100 rounded-2xl flex items-center justify-center font-black text-4xl text-slate-400">{selectedStudent.firstName[0]}</div>
                <div>
                   <h3 className="text-3xl font-black uppercase tracking-tighter text-slate-900 italic">{selectedStudent.firstName} {selectedStudent.lastName}</h3>
              ...
              <div className="grid grid-cols-2 gap-8 border-t pt-8">
                ...
                <div><p className={labelClasses}>Dirección</p><p className="font-bold text-slate-600">{selectedStudent.address}</p></div>
                <div><p className={labelClasses}>Fecha Nacimiento</p><p className="font-bold text-slate-700">{selectedStudent.birthDate || 'N/A'}</p></div>
              </div>
            </motion.div>
          </div>
        )}
        ...
        {paymentStudent && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative w-full max-w-lg bg-white rounded-[3rem] p-12 shadow-2xl">
               ...
               <h3 className="text-3xl font-black uppercase tracking-tighter mb-8 italic">REGISTRAR PAGO</h3>
               <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 mb-8">
                  <p className="text-[10px] font-black text-blue-600 uppercase mb-1">Saldo Pendiente</p>
                  <p className="font-black text-slate-900 text-3xl">S/ {paymentStudent.pending_balance}</p>
               </div>
               <form className="space-y-6">
                  <div><label className={labelClasses}>Monto a Abonar S/</label><input type="number" defaultValue={paymentStudent.pending_balance} className={inputClasses}/></div>
                  <button onClick={(e) => { e.preventDefault(); alert('Cobro registrado y saldo actualizado.'); setPaymentStudent(null); }} className="w-full py-5 bg-emerald-600 text-white rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl hover:scale-105 transition-all">Confirmar Transacción</button>
               </form>
            </motion.div>
          </div>
        )}
        ...
        {showRegisterModal && (
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative w-full max-w-5xl bg-white rounded-[3rem] shadow-2xl p-12 overflow-y-auto max-h-[90vh]">
              <div className="mb-8"><h2 className="text-3xl font-black text-slate-900 uppercase italic">Registro Manual - Athletic</h2></div>
              ...
              <button onClick={() => setShowRegisterModal(false)} className="absolute top-8 right-8 text-slate-300 hover:text-slate-900 transition-colors"><X size={32}/></button>
            </motion.div>
          </div>
        )}
      </AnimatePresence>
