...
import { Clock, Calendar, Target, Users, ArrowUpRight, UserRound, CheckCircle2 } from 'lucide-react';
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      <div className="text-center max-w-2xl mx-auto mb-16">
        ...
        <p className="text-slate-500 font-medium text-lg">Grupos reducidos para una enseñanza personalizada y eficiente.</p>
      </div>
        ...
        >
          {activeSchedules.map((s) => (
            <motion.div 
              key={s.id}
              whileHover={{ y: -10 }}
              className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-lg hover:shadow-2xl transition-all flex flex-col h-full group"
            >
              <div className="mb-6">
                <div className="flex justify-between items-center mb-4">
                  <span className="px-3 py-1 bg-slate-100 rounded-full text-[10px] font-bold text-slate-600 uppercase">Élite</span>
                  <Users size={18} className="text-slate-300 group-hover:text-blue-500 transition-colors" />
                </div>
                <h3 className="text-2xl font-black text-slate-900 leading-tight mb-2 truncate">{s.category}</h3>
                <p className="text-blue-600 font-black text-sm uppercase">{s.age}</p>
              </div>
              ...
              <div className="mt-auto">
                ...
                <a 
                  href="#register" 
                  className={`block w-full py-4 rounded-2xl font-black text-center text-sm transition-all shadow-md active:scale-95 ${activeTab === 'LMV' ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-emerald-600 text-white hover:bg-emerald-700'}`}
                >
                  RESERVAR CUPO
                </a>
              </div>
